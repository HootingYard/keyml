...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
    ...
    <!-- This performs tests on KeyML XHTML files that the keyml.dtd cannot.
    ...
    <sch:pattern id="links">
        ...
        <sch:rule context="// xhtml:a [@class = 'hootingyard']">
            <sch:assert test="regexp:test(@href, 'https?://(www\.)?hootingyard\.org.*|https://hootingyard\.github\.io.*')">
        ...
        <sch:rule context="// xhtml:a [@class = 'external-image']">
            <sch:assert test="regexp:test(@href, 'https?://\.(png|jpg|gif|jpeg)', 'i')">
                All external-image links point to image files online.
            </sch:assert>
        </sch:rule>

        <sch:rule context="// xhtml:a [@class = 'external-pdf']">
            <sch:assert test="regexp:test(@href, 'https?://.+\.pdf')">
                All external-pdf links point to PDF files online.
            </sch:assert>
        </sch:rule>

        <sch:rule context="// xhtml:a [@class = 'external-audio']">
            <sch:assert test="regexp:test(@href, 'https?://.+\.(wav|aif|mp3|mid)')">
                All external-audio links point to audio files online.
            </sch:assert>
        </sch:rule>

        <sch:rule context="// xhtml:a [@class = 'external-audio']">
            <sch:assert test="@title">
                External audio links must have a title for the player to use.
            </sch:assert>
        </sch:rule>
